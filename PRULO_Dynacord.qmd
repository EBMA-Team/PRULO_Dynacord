...
# STROBEInput1 <- STROBEInput |> dplyr::mutate(
#   PatientID = stringr::str_split_i(TreatmentUID,"\\.",1),
#   CombID = paste0(PatientID,AffectedSide)
# ) |> dplyr::filter(
#   stringr::str_detect(ImplantCodes,paste(IncludeAny,collapse = "|"))) |> 
...
Mastersheet <- SnapshotComb1 |> dplyr::select(
  Cohort,
...
ComplicTable1 <- ComplicTable |> rename(
  TreatmentUID = `Treatment ID`) |> left_join(
    ...
    OccurDelay = as.numeric(as.duration(interval(ymd(DateTreatment), `Date of Occurrence`)),"days"),
...
The sample overall had a mean follow up of `{r} round(mean(Mastersheet2$TreatDuration),1)` weeks.
...
A summary of the adverse events observed in the cohort is included in @tbl-complications.
