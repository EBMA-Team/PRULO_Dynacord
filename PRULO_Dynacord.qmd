...
ComplicTable1 <- ComplicTable |> rename_with(
 ...
 TreatmentUID = "TreatmentID") |> left_join(
  ...
  stringr::str_detect(str_to_lower(ComplicationOccurrence),"yes|possible")  & !is.na(CombID)
)
...
ComplicTable2 <- ComplicTable1 |> bind_rows(
  ...
  IntraComp1 |> dplyr::select(
    ...
    TreatmentUID %in% Mastersheet2$TreatmentUID & (DateofOccurrence < ymd(CurrentDate))
    )
...
# Filter ComplicMaster for non-censored categories
ComplicTRUE <- ComplicMaster |>
  filter(Category %in% CategoryNoCensor)

# Create wide format with presence/absence for each category
ComplicMasterInc <- Mastersheet |>
  distinct(CombID) |>
  cross_join(tibble(Category = CategoryNoCensor)) |>
  left_join(
    ComplicTRUE |>
      distinct(CombID, Category) |>
      mutate(present = TRUE),
    by = c("CombID", "Category")
  ) |>
...
ComplicIncTable <- ComplicMasterInc |>
  select(-CombID) |>
  tbl_summary(
...
The cohort displayed retear of N = {inline_text(ComplicIncTable, variable = RepairFailure,column = "stat_0")}% with (95%CI {inline_text(ComplicIncTable, variable = RepairFailure,column = "ci_stat_0")})%, as well as infection (N = {inline_text(ComplicIncTable, variable = Infection,column = "stat_0")}%, 95%CI {inline_text(ComplicIncTable, variable = Infection,column = "ci_stat_0")}%), implant removal (N = {inline_text(ComplicIncTable, variable = ImplantRemoval,column = "stat_0")}%, 95%CI {inline_text(ComplicIncTable, variable = ImplantRemoval,column = "ci_stat_0")}%) and capsulitis (N = {inline_text(ComplicIncTable, variable = Capsulitis,column = "stat_0")}%, 95%CI {inline_text(ComplicIncTable, variable = Capsulitis,column = "ci_stat_0")}%).
...
Table 10: Summary of cumulative incidences of adverse events after rotator cuff repair with suture of interest
...
```{r CmpRsk Plot 1}
Figure7 <- ggcuminc(CRModelRCR, outcome = c("Capsulitis","Infection","RepairFailure","ImplantRemoval")) +
  ...
  #theme_ggsurvfit_KMunicate() +
    ...
    scale_color_discrete(labels = c("Capsulitis", "Implant Removal", "Infection", "RepairFailure")) +
  scale_linetype_discrete(labels = c("Capsulitis", "Implant Removal", "Infection", "RepairFailure")) +
  scale_x_continuous(
    limits = c(0, 158)) +
  scale_y_continuous(
    #limits = c(0, 0.35),
